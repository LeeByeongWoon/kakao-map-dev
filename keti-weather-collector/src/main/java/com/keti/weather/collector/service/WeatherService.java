...
import com.keti.weather.collector.component.ProducerKafka;
...
public class WeatherService {
    ...
    @Value("${spring.weatherApi.url}")
    private String url;

    @Value("${spring.weatherApi.service-key}")
    ...
    public List<JSONObject> getRequestPointData(List<Map<String, Object>> queryParams) throws Exception {
        ...
        for(int cnt=0; cnt<queryParamsSize; cnt++) {
            ...
            if(statusCodeValue >= 200 && statusCodeValue <= 300) { 
                ...
                weatherDatas.add(new JSONObject(weatherData));
            }
        }
    ...
    public List<JSONObject> getRealTimeData(List<int[]> points) throws Exception {
        List<JSONObject> weatherDatas = null;
        ...
        for (int[] point : points) {
            ...
            queryParam.put("nx", point[0]);
            queryParam.put("ny", point[1]);
            
            queryParams.add(queryParam);
        }

        weatherDatas = getRequestPointData(queryParams);

        return weatherDatas;
    }


    public List<List<JSONObject>> getLeapTimeData(List<int[]> points) throws Exception {
        ...
        for(List<Map<String, Object>> minusQueryParam : minusQueryParams) {
            List<JSONObject> weatherDatas = getRequestPointData(minusQueryParam);

            leapDataList.add(weatherDatas);
        }
